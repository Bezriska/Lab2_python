    ...
    for object in os.listdir(active_path):
        ...
        if pathlib.Path(path).is_file():
            lst_file = [pathlib.Path(path).name,
                         ...
                         datetime.fromtimestamp(os.stat(pathlib.Path(path)).st_ctime).strftime("%d.%m.%Y"),
    ...
    if "\\" in target:
        if os.path.exists(object_path) and os.path.isdir(object_path):
            if os.path.exists(target) and os.path.isdir(target):
                shutil.copytree(object_path, pathlib.Path(target) / pathlib.Path(object), copy_function=shutil.copy2)
                print("Копирование завершено")
            else:
        ...
        if os.path.exists(object_path) and os.path.isdir(object_path):
            if os.path.exists(new_path) and os.path.isdir(new_path):
                shutil.copytree(object_path, new_path / object, copy_function=shutil.copy2)
                print("Копирование завершено")
            else:
                print(f"Не существует директория {new_path}")
        else:
...
def move(active_path, object, target):
    ...
    if "\\" in target:
        if os.path.exists(object_path):
            if os.path.exists(target) and os.path.isdir(target):
                shutil.move(object_path, pathlib.Path(target) / pathlib.Path(object))
